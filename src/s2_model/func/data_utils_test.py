...
def load_data_test(seed):
    """Load testing data using seed."""
    surv_pred_test = pd.read_csv(f"F:/repro/UKB_meno_pre/imputation/impute_merge/surv_pred/surv_pred_test_{seed}.csv")
    return surv_pred_test

def load_variable_definitions_test():
...
def load_lasso_features_test(seed):
    """Load LASSO-selected features for test data."""
    pro_lasso = pd.read_csv(f"F:/repro/UKB_meno_pre/s1_lasso/lasso_result/surv_pred_train/surv_pred_train_{seed}.csv")
    return pro_lasso


def prepare_data_test(surv_pred_test, pro_lasso, out_surv, predictor='pro', exp_var=None):
    ...
    if (predictor in ['pro_clin', 'clin']) and exp_var is None:
        exp_var, _, _ = load_variable_definitions_test()
    
    # Extract features based on predictor type
    if predictor == 'pro':
        # Proteomics features only
        X = surv_pred_test[[col for col in pro_lasso['lasso_pro'] if col in surv_pred_test.columns] + ['region_code_10']].copy()
    elif predictor == 'pro_clin':
        # Proteomics + clinical features
        X = surv_pred_test[[col for col in pro_lasso['lasso_pro'] if col in surv_pred_test.columns] + 
                           [col for col in exp_var['Variable'] if col in surv_pred_test.columns and col not in ['eid', 'region_code_ori']]].copy()
    elif predictor == 'clin':
        # Clinical features only
        X = surv_pred_test[[col for col in exp_var['Variable'] if col in surv_pred_test.columns and col not in ['eid', 'region_code_ori']]].copy()
    
    # Extract outcomes
    Y_df = surv_pred_test[[col for col in out_surv['Variable'] if col in surv_pred_test.columns]]
    
    # Process categorical variables for clinical predictors
    if predictor in ['pro_clin', 'clin']:
        X = _process_categorical_variables_test(X)
...
def _process_categorical_variables_test(X):
