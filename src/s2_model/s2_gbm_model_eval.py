...
# 参数解析
parser = argparse.ArgumentParser(description='GBM Survival Model Evaluation')
...
model_path = os.path.join(model_dir, f"gbm_{predictor}_best_model_{seed}.pkl")
...
# 保存评估指标为CSV文件
pd.DataFrame({'C_index': [c_index]}).to_csv(os.path.join(model_dir, f"gbm_{predictor}_cindex_{seed}.csv"), index=False)
score_brier.to_frame().to_csv(os.path.join(model_dir, f"gbm_{predictor}_ibs_{seed}.csv"))
score_brier_years.to_frame().to_csv(os.path.join(model_dir, f"gbm_{predictor}_brier_years_{seed}.csv"))
