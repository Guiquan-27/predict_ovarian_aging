...
# 参数解析
parser = argparse.ArgumentParser(description='CoxPH Survival Model Evaluation')
parser.add_argument('--seed', type=int, default=3456, help='Random seed for reproducibility')
parser.add_argument('--predictor', type=str, choices=['pro', 'pro_clin', 'clin'], default='pro_clin', help='Predictor type')
...
model_dir = os.path.join(output_dir, f"model/{predictor}/coxph")
model_path = os.path.join(model_dir, f"coxph_{predictor}_best_model_{seed}.pkl")
...
if hasattr(loaded_model, 'coef_'):
    coefficients = pd.Series(loaded_model.coef_, index=X_test_model.columns)
    print("\n模型系数 (前10个):", flush=True)
...
score_brier = pd.Series(
    ...
    index=["CoxPH", "Random", "Kaplan-Meier"],
    name="IBS",
)
print(score_brier)
...
# 保存评估指标为CSV文件
pd.DataFrame({'C_index': [c_index]}).to_csv(os.path.join(model_dir, f"coxph_{predictor}_cindex_{seed}.csv"), index=False)
score_brier.to_frame().to_csv(os.path.join(model_dir, f"coxph_{predictor}_ibs_{seed}.csv"))
score_brier_years.to_frame().to_csv(os.path.join(model_dir, f"coxph_{predictor}_brier_years_{seed}.csv"))
